...
DashboardView::DashboardView(DashboardVm& vm, aria::binding::BindingEngine& be)
    ...
    UIButton* mountToggleBtn = wb::ios::ui::make_button(@"");
    UILabel*  mountStatusLbl = wb::ios::ui::make_label(@"");
    ...
    UIStackView* top = [[UIStackView alloc] initWithArrangedSubviews:@[
        title, info, mountStatusLbl, modalCartBtn, windowCartBtn, backBtn, mountToggleBtn
    ]];
    ...
    // Placeholder shown inside the host while the extension slot is empty.
    UILabel* placeholder = wb::ios::ui::make_label(@"");
    ...
    [NSLayoutConstraint activateConstraints:@[
        [NSLayoutConstraint constraintWithItem:placeholder
    ...
    be.bind_text_oneway(vm.mountStatus,      wb::ios::ui::view_for(mountStatusLbl));
    ...
    be.bind_text_oneway(vm.mountToggleLabel, wb::ios::ui::view_for(mountToggleBtn));
    ...
    be.bind_command(vm.mountToggle, wb::ios::ui::view_for(mountToggleBtn));

    // ── Extension point (mount) rendering ──────────────────────────────
    // The host area shows the mounted provider's UI (e.g. cart), or the
    // placeholder when the slot is empty.
    auto& mountedVm = vm.mountedVm;
    auto render_mount = [this, &vm, &be, placeholder](
        ...
        placeholder.hidden = (vmp != nullptr);
        if (!vmp) return;
        UIViewController* page = wb::ios::UIViewFactory::instance().build(
    ...
    subscriptions_.push_back(mountedVm.on_changed(
        [render_mount](const std::shared_ptr<aria::binding::ViewModel>& v) {
            render_mount(v);
        }));
    render_mount(mountedVm.get());

    // ── Navigation rendering (modal / window) — independent of mount ──
